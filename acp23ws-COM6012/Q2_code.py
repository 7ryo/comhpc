...
for i, param_map in enumerate(paramGrid_elastic):
    ...
    elastic = param_map[logit_elastic.elasticNetParam]
    mean = avgMetrics_elastic[i]
    std = stdMetrics_elastic[i]
    results[elastic].append((reg, mean, std))

for enet in elas_netparam_list:
    data = sorted(results[enet])  # Sort by regParam
    regs, means, stds = zip(*data)
    plt.errorbar(regs, means, yerr=stds, fmt='-o', capsize=3, label=f'elasticNetParam={elastic}')

plt.xscale('log')
plt.xlabel('regParam')
plt.ylabel('avg AUC')
plt.legend()
plt.title('Logistic Regresion with elasticnet')
